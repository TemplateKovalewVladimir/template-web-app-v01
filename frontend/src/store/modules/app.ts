...
export const useAppStore = defineStore('app', {
  state: (): AppState => {
    return {
      ...
      mobile: false, // 是否是移动端
      title: import.meta.env.APP_TITLE, // 标题
      ...
      currentSize: getStorage('currentSize') || 'small', // 组件尺寸
  ...
  actions: {
    ...
    setLayout(layout: LayoutType) {
      if (this.mobile && layout !== 'classic') {
        ElMessage.warning('Переключение на другие раскладки не поддерживается в мобильном режиме.')
        return
      }
      this.layout = layout
